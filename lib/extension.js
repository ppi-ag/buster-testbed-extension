...
function findJsTestFiles(rootPath, testPattern) {
    return glob({
        cwd: rootPath,
        strict: false
    }, testPattern);
...
function createTestPatterns(testbeds, testsOption) {
...
function addConfigGroups(group, config, testbeds, testPatterns, strict) {

    var i;
    for (i = 0; i < testbeds.length; i++) {
        if (strict !== false || findJsTestFiles(group.rootPath, [ testPatterns[i] ]).length > 0) {
            config.addGroup(testbeds[i], {
                "extends": group.name,
                testbed: testbeds[i],
                tests: [testPatterns[i]]
            }).source = group.source;
        }
    }
}
...
module.exports = {
    ...
    preConfigure: function (group, config) {
        ...
        var testPatterns = createTestPatterns(testbeds, this.options.tests);
        addConfigGroups(group, config, testbeds, testPatterns, this.options.strict);
    }
};
